...
public class TermDAO {
	...
	public void register(RegisterTerm tr) {
		...
		try {
			...
			ps.setInt(4, Util.returnKey("numero", "setor", tr.getJcSector().getSelectedItem().toString())); // chave
																											// estrageira
																											// de setor
			...
			ps.setInt(9, Util.returnKey("numero", "setor", tr.getJcSector().getSelectedItem().toString())); // chave
																											// estrageira
																											// de setor
	...
	public List<String> getList() {
		...
		try {
			...
			while (rs.next()) {
				trs.add(Util.convertUp(
						"COMPONENTE: " + rs.getString("obj.nome") + "     SETOR: " + rs.getString("s.nome")));
			}
			ps.close();
		} catch (SQLException e) {
			throw new RuntimeException(e);
	...
	public List<String> getTerm(ConsultTerm c, int key) {
		List<String> trs = new ArrayList<>();
		String sql = "select obj.nome, s.nome, tr.situacao, tr.descricao, tr.funcCriaMat, tr.dataCriacao from termoresponsabilidade as tr, objetotermo  as ot, responsabilidade as r, objeto as obj , setor as s where tr.numero = "
				+ key
				+ " and tr.numero = ot.termoNum and ot.objCod = obj.codigo and ot.objCod = r.objCod and tr.setNum = r.setNum";
		try {
			PreparedStatement ps = connection.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				...
				setMenssage(497, 307, Util.getValue("nome", "funcionario", "matricula", rs.getString("tr.funcCriaMat")),
						c);
	...
	public List<String> getUpdate(UpdateTerm u, int key) {
		List<String> trs = new ArrayList<>();
		String sql = "select obj.nome, s.nome, s.numero, tr.situacao, tr.descricao, tr.funcCriaMat, tr.dataCriacao from termoresponsabilidade as tr, objetotermo  as ot, responsabilidade as r, objeto as obj , setor as s where tr.numero = "
				+ key
				+ " and tr.numero = ot.termoNum and ot.objCod = obj.codigo and ot.objCod = r.objCod and tr.setNum = r.setNum";
		try {
			PreparedStatement ps = connection.prepareStatement(sql);
			...
			while (rs.next()) {
				 ...
				 * 
				 * setMenssage(497, 307, Util.getValue("nome", "funcionario", "matricula",
				 * rs.getString("tr.funcCriaMat")), u);
				 * 
	...
	public void getDelete(DeleteTerm d, int key) {
		...
		d.setsNumber(key);
		String sql = "select obj.nome, s.nome, obj.codigo, tr.numero from termoresponsabilidade as tr, objetotermo as ot, responsabilidade as r, objeto as obj , setor as s where tr.numero = "
				+ key
				+ " and tr.numero = ot.termoNum and ot.objCod = obj.codigo and ot.objCod = r.objCod and tr.setNum = r.setNum";
		try {
			PreparedStatement ps = connection.prepareStatement(sql);
			...
			while (rs.next()) {
				...
				d.setoCod(rs.getInt("obj.codigo"));
				d.settNumber(rs.getInt("tr.numero"));
	...
	public void delete(DeleteTerm d) {
		String sql = "call sp_DeletaTermo(?, ?, ?);";
		try {
			PreparedStatement ps = connection.prepareStatement(sql);

			ps.setInt(1, d.gettNumber());
			ps.setInt(2, d.getoCod());
			ps.setInt(3, d.getsNumber());
	...
	public void update(UpdateTerm u) {
		...
		try {
			...
			ps.setInt(5, Util.matriculation); // ERRADOOOOOOOOOO
